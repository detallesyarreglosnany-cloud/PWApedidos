...
> **Datos sensibles fuera del repositorio.** Por eso la lista de precios
...
- Pedidos y hojas se combinan **campo por campo**: cada documento guarda en `fv` la hora del último cambio de cada campo (`DB.stampFields` al guardar, `mergeFields` en el cliente y en el servidor). Si dos equipos cambian el mismo pedido u hoja sin haberse sincronizado (una PC saca un cliente a espera y la otra pone el despachador), se conservan los dos cambios. Los campos que van juntos se toman todos del mismo lado: en el pedido `status, loadId, locked, loadStatusName, noteNumber, loadNumber, dispatchedAt, heldAt`; en la hoja `status, statusHistory, number, closedAt, approvedAt, totals, firstNote, lastNote`. El teléfono del vendedor no marca los campos de estado/hoja de sus pedidos: esos solo los decide la oficina. Un documento sin `fv` (de una versión anterior) vale entero con su `updatedAt`.
- Los pedidos de una hoja son los que dicen pertenecer a ella (`o.loadId`); `orderIds` solo da el orden de las columnas. Un pedido nunca aparece en dos hojas ni en una hoja y en espera a la vez.
...
- Hojas automáticas sin duplicados: la hoja que arma la oficina para un pedido tiene un id fijo (`l_auto_<pedido>`), así dos equipos de oficina que arman a la vez crean la misma hoja; además cada oficina ajusta `orderIds` a lo que dicen los pedidos y devuelve a la cola un pedido cuya hoja ya no existe (`Loads.repair`, solo sobre lo que lleva 2 minutos sin cambios).
- Reiniciar datos (`/api/pedidos/reset`, clave admin, Ajustes → Reiniciar datos): borra pedidos, hojas, historial y numeración (o todo) y cambia la época (`DistSetting.epoch`). Cada equipo manda su época al sincronizar; si no coincide, el servidor no escribe nada y responde `{ reset }`: el equipo borra su copia local y baja todo de nuevo; lo no subido de antes del reinicio se descarta y lo creado después (`resetAt`, sin señal) se conserva y se sube. Si la app se cierra a mitad, el borrado se retoma (`resetPending`). Una pestaña con datos de otra época no guarda nada: se recarga. El borrado toma un candado exclusivo de Postgres y cada escritura de sync uno compartido que vuelve a comprobar la época, así nada anterior al reinicio se cuela después. `/api/pedidos/numbers` también rechaza una época vieja.
- Avisos push (`/api/pedidos/push`, tablas `DistPush` y `DistSetting`): el servidor genera sus claves VAPID la primera vez (nada que configurar en Vercel) y avisa después de responder la sincronización (`after`). Tag del aviso = `o-<pedido>-<evento>`, el mismo que el aviso local: nunca sale doble.
- El vendedor baja las hojas de carga donde están sus clientes (solo lectura) y los últimos 60 días de pedidos al configurar un teléfono nuevo.
