...
- Hojas automáticas sin duplicados: la hoja que arma la oficina para un pedido tiene un id fijo (`l_auto_<pedido>`), así dos equipos de oficina que arman a la vez crean la misma hoja; además cada equipo repara solo cualquier pedido que figure en dos hojas (`Loads.repair`).
